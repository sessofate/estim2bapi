...
@pytest.mark.parametrize("slA,elA,slB,elB,dur,mode,power,block", [(0,15,0,50,2,None,"H",False),
                                                                  (0,15,0,50,1,None,"L",False)])
def test_rampAB(slA, elA, slB, elB, dur, mode, power, block):
    e2b = Estim(recv_responses=True, dryrun=False, verbose=2)
    e2b.start()
